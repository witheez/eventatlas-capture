...
describe('lookupUrl', () => {
  ...
  it('should return local match in bulk_only mode', async () => {
    ...
    const result = await lookupUrl('https://example.com/page', { ...apiSettings, syncMode: 'bulk_only' });
  ...
  it('should return local match for organizer links', async () => {
    ...
    const result = await lookupUrl('https://example.com/org', { ...apiSettings, syncMode: 'bulk_only' });
  ...
  it('should return null when no API credentials and no local match', async () => {
    const result = await lookupUrl('https://example.com/page', { apiUrl: '', apiToken: '', syncMode: 'hybrid' });
    expect(result).toBeNull();
  });
});
