  ...
  return (
    <div class="upload-queue" style={{ display: 'block' }}>
      ...
      <div class="upload-queue-items">
        {items.map((item) => (
          <UploadQueueItem key={item.id} item={item} onRetry={onRetry} />
        ))}
      </div>
    </div>
  );
...
function UploadQueueItem({ item, onRetry }: UploadQueueItemProps): preact.JSX.Element {
  ...
  return (
    <div class={statusClass} data-queue-id={item.id}>
      ...
      {item.status === 'complete' && <span class="upload-queue-check">{'\u2713'}</span>}
      {item.status === 'failed' && (
        <button class="upload-queue-retry" title="Retry upload" onClick={handleRetryClick}>
          {'\u21BB'}
        </button>
      )}
